const filePath = "/carData.json";

// fetch the raw data from carData.json
async function fetchCarData() {
	return [
		{
			name: "Audi",
			models: [
				{
					name: "A8",
					year: "2020",
					img: "Audi-A8.png",
					specs: {
						engine: {
							max_hp: "335 @ 5.0k",
							torque: "369 @ 1.4k",
						},
						body: {
							type: "Sedan",
							num_door: "4",
							num_seats: "5",
							curb_weight: "4773",
						},
						fuel: {
							fuel_type: "gas",
							combined_mpg: "22",
							tank_size: "21.7",
						},
						technicals: {
							msrp: "$80,000",
							trans: "Auto",
							drive_type: "RWD",
						},
					},
				},
				{
					name: "e-tron Sportback",
					year: "2020",
					img: "Audi-E-tron.png",
					specs: {
						engine: {
							max_hp: "400 @ 7.0k",
							torque: "490 @ 3.4k",
						},
						body: {
							type: "SUV",
							num_door: "5",
							num_seats: "5",
							curb_weight: "5787",
						},
						fuel: {
							fuel_type: "EV",
							combined_mpg: null,
							tank_size: null,
						},
						technicals: {
							msrp: "$75,000",
							trans: "Auto",
							drive_type: "FWD",
						},
					},
				},
				{
					name: "Q3",
					year: "2020",
					img: "Audi-Q3-Sportback.png",
					specs: {
						engine: {
							max_hp: "184 @ 4.8k",
							torque: "221 @ 3.5k",
						},
						body: {
							type: "SUV",
							num_door: "5",
							num_seats: "5",
							curb_weight: "3902",
						},
						fuel: {
							fuel_type: "gas",
							combined_mpg: "25",
							tank_size: "15.9",
						},
						technicals: {
							msrp: "$40,000",
							trans: "Auto",
							drive_type: "RWD",
						},
					},
				},
			],
		},
		{
			name: "Bentley",
			models: [
				{
					name: "Bentayga",
					year: "2020",
					img: "Bentley-Bentayga.png",
					specs: {
						engine: {
							max_hp: "626 @ 5.0k",
							torque: "664 @ 1.7k",
						},
						body: {
							type: "SUV",
							num_door: "5",
							num_seats: "5",
							curb_weight: "5529",
						},
						fuel: {
							fuel_type: "gas",
							combined_mpg: "14",
							tank_size: "22.5",
						},
						technicals: {
							msrp: "$200,000",
							trans: "Auto",
							drive_type: "FWD",
						},
					},
				},
				{
					name: "Continental",
					year: "2020",
					img: "Bentley-Continental.png",
					specs: {
						engine: {
							max_hp: "542 @ 6.0k",
							torque: "568 @ 2.0k",
						},
						body: {
							type: "Coupe",
							num_door: "2",
							num_seats: "4",
							curb_weight: "4773",
						},
						fuel: {
							fuel_type: "gas",
							combined_mpg: "19",
							tank_size: "24",
						},
						technicals: {
							msrp: "$260,000",
							trans: "Auto",
							drive_type: "RWD",
						},
					},
				},
				{
					name: "Flying Spur",
					year: "2020",
					img: "White-Bentley-Flying-Spur.png",
					specs: {
						engine: {
							max_hp: "542 @ 6.0k",
							torque: "568 @ 2.0k",
						},
						body: {
							type: "Sedan",
							num_door: "4",
							num_seats: "5",
							curb_weight: "5137",
						},
						fuel: {
							fuel_type: "gas",
							combined_mpg: "14",
							tank_size: "23.8",
						},
						technicals: {
							msrp: "$210,000",
							trans: "Auto",
							drive_type: "FWD",
						},
					},
				},
			],
		},

		{
			name: "BMW",
			models: [
				{
					name: "4 Series",
					year: "2018",
					img: "2017-bmw-4-series.png",
					specs: {
						engine: {
							max_hp: "473 @ 6.2k",
							torque: "400 @ 2.6k",
						},
						body: {
							type: "Coupe",
							num_door: "2",
							num_seats: "4",
							curb_weight: "3830",
						},
						fuel: {
							fuel_type: "gas",
							combined_mpg: "19",
							tank_size: "15.6",
						},
						technicals: {
							msrp: "$60,000",
							trans: "Auto",
							drive_type: "RWD",
						},
					},
				},
				{
					name: "6 Series Gran Coupe",
					year: "2018",
					img: "bmw-6-series.png",
					specs: {
						engine: {
							max_hp: "443 @ 5.5k",
							torque: "470 @ 2.0k",
						},
						body: {
							type: "Sedan",
							num_door: "4",
							num_seats: "5",
							curb_weight: "4570",
						},
						fuel: {
							fuel_type: "gas",
							combined_mpg: "23",
							tank_size: "18.5",
						},
						technicals: {
							msrp: "$40,000",
							trans: "Auto",
							drive_type: "FWD",
						},
					},
				},
				{
					name: "7 Series",
					year: "2018",
					img: "bmw-7-series.png",
					specs: {
						engine: {
							max_hp: "600 @ 5.5k",
							torque: "620 @ 1.5k",
						},
						body: {
							type: "Sedan",
							num_door: "4",
							num_seats: "5",
							curb_weight: "5156",
						},
						fuel: {
							fuel_type: "gas",
							combined_mpg: "16",
							tank_size: "20.6",
						},
						technicals: {
							msrp: "$80,000",
							trans: "Auto",
							drive_type: "RWD",
						},
					},
				},
			],
		},
		{
			name: "Ford",
			models: [
				{
					name: "Expedition",
					year: "2019",
					img: "ford-Expedition.png",
					specs: {
						engine: {
							max_hp: "380 @ 5.0k",
							torque: "470 @ 2.2k",
						},
						body: {
							type: "SUV",
							num_door: "5",
							num_seats: "8",
							curb_weight: "5306",
						},
						fuel: {
							fuel_type: "gas",
							combined_mpg: "19",
							tank_size: "23.2",
						},
						technicals: {
							msrp: "$57,000",
							trans: "Auto",
							drive_type: "FWD",
						},
					},
				},
				{
					name: "F-150",
					year: "2019",
					img: "Ford-f150.png",
					specs: {
						engine: {
							max_hp: "290 @ 6.5k",
							torque: "265 @ 4.0k",
						},
						body: {
							type: "Pickup",
							num_door: "2",
							num_seats: "3",
							curb_weight: "4021",
						},
						fuel: {
							fuel_type: "gas",
							combined_mpg: "21",
							tank_size: "23",
						},
						technicals: {
							msrp: "$50,000",
							trans: "Auto",
							drive_type: "AWD",
						},
					},
				},
				{
					name: "Explorer",
					year: "2019",
					img: "Ford-Explorer.png",
					specs: {
						engine: {
							max_hp: "400 @ 5.5k",
							torque: "415 @ 3.5k",
						},
						body: {
							type: "SUV",
							num_door: "5",
							num_seats: "6",
							curb_weight: "4701",
						},
						fuel: {
							fuel_type: "gas",
							combined_mpg: "21",
							tank_size: "20",
						},
						technicals: {
							msrp: "$40,000",
							trans: "Auto",
							drive_type: "AWD",
						},
					},
				},
			],
		},

		{
			name: "Honda",
			models: [
				{
					name: "Civic",
					year: "2018",
					img: "honda-civic.png",
					specs: {
						engine: {
							max_hp: "158 @ 6.5k",
							torque: "138 @ 4.2k",
						},
						body: {
							type: "Sedan",
							num_door: "4",
							num_seats: "5",
							curb_weight: "2870",
						},
						fuel: {
							fuel_type: "gas",
							combined_mpg: "35",
							tank_size: "12.4",
						},
						technicals: {
							msrp: "$20,000",
							trans: "Auto",
							drive_type: "FWD",
						},
					},
				},
				{
					name: "Accord",
					year: "2018",
					img: "blue-honda-accord.png",
					specs: {
						engine: {
							max_hp: "192 @ 5.5k",
							torque: "190 @ 1.6k",
						},
						body: {
							type: "Sedan",
							num_door: "4",
							num_seats: "5",
							curb_weight: "3150",
						},
						fuel: {
							fuel_type: "gas",
							combined_mpg: "33",
							tank_size: "14.8",
						},
						technicals: {
							msrp: "$28,000",
							trans: "Auto",
							drive_type: "FWD",
						},
					},
				},
				{
					name: "CR-V",
					year: "2018",
					img: "honda-crv.png",
					specs: {
						engine: {
							max_hp: "190 @ 5.6k",
							torque: "179 @ 2.0k",
						},
						body: {
							type: "SUV",
							num_door: "5",
							num_seats: "5",
							curb_weight: "3337",
						},
						fuel: {
							fuel_type: "gas",
							combined_mpg: "30",
							tank_size: "14",
						},
						technicals: {
							msrp: "$30,000",
							trans: "Auto",
							drive_type: "FWD",
						},
					},
				},
			],
		},
		{
			name: "Jeep",
			models: [
				{
					name: "Renegade",
					year: "2019",
					img: "Jeep-Renegade.png",
					specs: {
						engine: {
							max_hp: "177 @ 5.7k",
							torque: "210 @ 1.7k",
						},
						body: {
							type: "SUV",
							num_door: "5",
							num_seats: "5",
							curb_weight: "3161",
						},
						fuel: {
							fuel_type: "gas",
							combined_mpg: "27",
							tank_size: "12.7",
						},
						technicals: {
							msrp: "$30,000",
							trans: "Auto",
							drive_type: "AWD",
						},
					},
				},
				{
					name: "Wrangler",
					year: "2019",
					img: "Jeep-Wrangler.png",
					specs: {
						engine: {
							max_hp: "285 @ 6.4k",
							torque: "260 @ 4.8k",
						},
						body: {
							type: "SUV",
							num_door: "3",
							num_seats: "4",
							curb_weight: "3948",
						},
						fuel: {
							fuel_type: "gas",
							combined_mpg: "20",
							tank_size: "17.5",
						},
						technicals: {
							msrp: "$35,000",
							trans: "Auto",
							drive_type: "AWD",
						},
					},
				},
				{
					name: "Cherokee",
					year: "2019",
					img: "Jeep-Cherokee.png",
					specs: {
						engine: {
							max_hp: "180 @ 6.4k",
							torque: "171 @ 4.6k",
						},
						body: {
							type: "SUV",
							num_door: "5",
							num_seats: "5",
							curb_weight: "3710",
						},
						fuel: {
							fuel_type: "gas",
							combined_mpg: "25",
							tank_size: "15.8",
						},
						technicals: {
							msrp: "$40,000",
							trans: "Auto",
							drive_type: "AWD",
						},
					},
				},
			],
		},
	];

	/*
	
	try {
		const response = await fetch(filePath);

		if (!response.ok) {
			throw new Error(`Fetch error!: ${response.status}`);
		}

		const data = await response.json();
		return data.makes;
	} catch (error) {
		console.error("getCarData fetch Error:", error);
	}
	*/
}

// fetch data for a specific car
async function getCarData(make, model) {
	try {
		const response = await fetchCarData();

		if (!response.ok) {
			throw new Error(`Fetch error!: ${response.status}`);
		}

		// const data = await response.json;
		const carData = response;

		for (let i = 0; i < carData.length; i++) {
			const currentMakeIteration = carData[i].name;

			if (currentMakeIteration === make) {
				for (let x = 0; x < carData[i].models.length; x++) {
					const currentModelIteration = carData[i].models[x].name;

					if (currentModelIteration === model) {
						return carData[i].models[x];
					}
				}
				break;
			}
		}
	} catch (error) {
		console.error("getCarData fetch Error:", error);
	}
}

export { getCarData, fetchCarData };
